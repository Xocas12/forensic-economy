# forensic-economy — top-level targets. Requires `uv` (https://docs.astral.sh/uv/) and GNU make.
UV ?= uv
...
help:
	...
	@echo "make test             run every test suite (shared library + three projects)"
	@echo "make lint             ruff check + format check"
	@echo "make data             run `make data` in every project (never fabricates; fails loudly per source)"
	@echo "make data-<project>   one project: aaer | china | gosplan"
